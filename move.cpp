...
{
	//if(FAILED(CreateSpriteTexture(m_pd3dDevice,_T("ghost_m.png"),ch_X*13,che_Y,D3DCOLOR_XRGB(0,0,0),ghost_texture)))
	 if(FAILED(CreateSpriteTexture(m_pd3dDevice,_T("ghost_m.png"),ch_x,ch_y,D3DCOLOR_XRGB(0,0,0),ghost_texture)))
	 {
...
void move :: rightmove(float &ghostX,float &ghostY)
{
	if(ghostX<=585)
	{
		...
		{
			crush.obj_check(ghostX,ghostY,check);
			if(check==0)
			{
				move_check++;
			if(move_check==21)
...
void move :: upmove(float &ghostX,float &ghostY)
{
	...
	if(ghostY>=0)
	{
		if(check==1)
		{
			if(ghostY>crush.objh[crush.cnt].vector.y+2)
			{
				move_check++;	
				if(move_check==56)
					move_check=41;
...
D3DXVECTOR3 move :: jump(float &ghostX, float &ghostY,int cnt)//로프에서 뛰면서 방향키->문제
{
	...
	if(ghostY<=500)
	{
		...
		if(crush.obj_check(ghostX,ghostY,check)==1 && (GetAsyncKeyState(VK_UP) & 0x8000||GetAsyncKeyState(VK_DOWN) & 0x8000))//점프중 오르내림
		{
		...
		{	
			...
			{
				high+=0.1;
				if(crush.obj_check(ghostX,ghostY,check)!=1)//로프에서 제자리 점프를 막기위해서
					ghostY+=(float)high;
			...
			else
			{
				if(cnt<49)
				{
					if(crush.obj_check(ghostX,ghostY,check)!=1)
						ghostY-=(float)high;
...
D3DXVECTOR3 move :: fall(float &ghostX, float &ghostY)
{
	...
	if(check==4)
	{
		while(crush.obj_check(ghostX,ghostY,check)==4)
		{
			move_check++;
			if(move_check==66)
				move_check=56;
...
{
	if(crush.obj_check(ghostX,ghostY,check)==2)
		return 2;
	else if(crush.obj_check(ghostX,ghostY,check)==2)
		return 2;
	else
		return 0;
}
...
{
	return crush.obj_check(ghostX,ghostY,check);
}